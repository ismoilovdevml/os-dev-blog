...
`GTK`: GIMP Toolkit (GTK) grafik interfeyslarni yaratish uchun multi-platform toolkitdir. `GIMP` loyihasining bir qismi sifatida ishlab chiqilgan, keyinchalik u `GNOME` desktop environmenti uchun asos sifatida qabul qilingan. U ko'plab mashhur dasturiy paketlarda qo'llaniladi va asosan C tilida dasturlashtirilgan.

`JavaFX/Swing`: Bular Java-da GUI yaratish uchun ikkita related toolkitdir. Swing ikkalasining eng oldingisi bo'lib, ko'p yillar davomida Java GUI ishlab chiqish uchun de-fakto standarti bo'lgan. JavaFX - bu Swing o'rnini bosish uchun mo'ljallangan zamonaviyroq toolkit. U CSS-ga o'xshash uslublar va hardware-accelerated grafika kabi zamonaviy xususiyatlarni qo'llab-quvvatlashni o'z ichiga oladi.
...
![GTK](/images/article/linux-de/4.png)